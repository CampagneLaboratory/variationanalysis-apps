...
main() {
    ...
    for file in ${HOME}/in/SBI/*.sbi; do
        ...
        dx-docker run \
            ...
            bash -c "source ~/.bashrc; cd $HOME/out/VEC; export-genotype-tensors.sh 2g --indel-sequence-length ${IndelSequenceLength} --feature-mapper ${FeatureMapper} -i \"/${HOME}/in/SBI/${SBI_basename}.sbi\" -o /${HOME}/out/${SBI_basename} --label-smoothing-epsilon ${LabelSmoothingEpsilon} --ploidy ${Ploidy} --genomic-context-length ${GenomicContextLength} --export-input input --export-output softmaxGenotype --export-output meta-data --sample-name \"${SampleName}\"  --sample-type germline"
